### Hey there! I'm Pooja, also DataTinker👋  
**Data Scientist | AI Enthusiast | Problem Solver**  

---

🔍 **About Me**
- 📊 Data scientist with a love for numbers, patterns, and making sense of chaos.
- 🤖 Passionate about AI, deep learning, and NLP (because making machines understand humans is cool!).
- ✨ On a mission to build intelligent solutions and (hopefully) take over the world... just kidding! 😆
- 🌎 Future globetrotter – working on my bucket list one step at a time.
- ☕ Runs on tea, curiosity, and a little bit of chaos!

---

📈 **GitHub Stats**
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Poojaschavan97&theme=radical&hide_border=true" alt="Streak Stats" />
  <br>
  <img src="https://github-readme-stats.vercel.app/api?username=Poojaschavan97&show_icons=true&theme=radical" alt="GitHub Stats" />
  <br>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Poojaschavan97&layout=compact&theme=radical" alt="Top Languages" />
</p>

---

🚀 **My Projects**

| Project | Description | Repo |
|---------|------------|------|
| 🏏 IPL Data Analysis | An interactive dashboard for IPL insights using Python & Streamlit | [Repo](https://github.com/Poojaschavan97/ML_project_ipl.git) |
| 💸 Loan Riskiness Prediction Model | An interactive model to predict risk using ML algorithm | [Repo](https://github.com/Poojaschavan97/Loan-Riskiness-Prediction-OG.git) |
|🗄️  Metal Detection CNN MODEL | A CNN Model Which can detect defects in metal |[Repo](https://github.com/Poojaschavan97/Metal-Detection-.git)
| 🗏  NLP_GUI | A GUI based Project | [Repo](https://github.com/Poojaschavan97/NLP__GUI.git) |
| 🗏  FLASKWEB | A project using nlp api's and flask | [Repo](https://github.com/Poojaschavan97/FLASKWEB.git) |

---

📫 **Get in Touch**
- 💼 [LinkedIn](https://www.linkedin.com/in/pooja-chavan-s27/)
- ✉️ Email: poojachavan.0109@gmail.com (for cool projects, not spam 😜)

---

👀 **Fun Fact:** If AI takes over, at least I’ll be the one debugging it! 🤖🔥
